...
import { balancesController } from '../../sdk-client.js';
import { stringifyRawJson } from '../../utils.js';
import { type ToolDefinition } from '../index.js';


interface GetBalancesMCPToolRequest {
  'version': string,
  'providerId': string,
...
async function executeGetBalances(
  req: GetBalancesMCPToolRequest
): Promise<CallToolResult> {
  try {
    const response = await balancesController.getBalances(
      req['version'],
      req['providerId'],
...
async function handleGetBalances(
  args: unknown | GetBalancesMCPToolRequest
): Promise<CallToolResult> {
  const validationSummary = schemas.validate(args, "Balances", "get-balances");
  if (!validationSummary.isValid) {
    ...
    return {
      content: [{ type: "text", text: "Invalid arguments for getBalances tool." }],
      isError: true,
    };
  }

  return await executeGetBalances(args as GetBalancesMCPToolRequest);
}

export async function balancesGetBalancesTool(): Promise<ToolDefinition> {
  return {
    tool: {
      name: "balances-getBalances",
...
\`mode\` is supported by a subset of providers. Log into the [Data Recipient Hub](https://recipient.ddp.akoya.com/login) and click [here](https://recipient.ddp.akoya.com/support/article/kA0Uw00000015GzKAI) to view a list of all providers supporting the \`mode\` parameter.`,
      inputSchema: await schemas.loadSchema("Balances", "get-balances"),
    },
