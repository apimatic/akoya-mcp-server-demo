...
async function executeGetAccounts(
  ...
  try {
    const response = await investmentsController.getAccounts(
      req['version'],
      req['providerId'],
...
async function handleGetAccounts(
  ...
  const validationSummary = schemas.validate(args, "Investments", "get-accounts");
...
export async function investmentsGetAccountsTool(): Promise<ToolDefinition> {
  return {
    tool: {
      name: "investments-getAccounts",
...
\`mode\` is supported by a subset of providers. Log into the [Data Recipient Hub](https://recipient.ddp.akoya.com/login) and click [here](https://recipient.ddp.akoya.com/support/article/kA0Uw00000015GzKAI) to view a list of all providers supporting the \`mode\` parameter.`,
      inputSchema: await schemas.loadSchema("Investments", "get-accounts"),
    },
