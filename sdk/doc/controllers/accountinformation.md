...
```ts
const accountinformationController = new AccountinformationController(client);
```
...
try {
  const { result, ...httpResponse } = await accountInformationController.getAccountsInfo(
  version,
  providerId,
